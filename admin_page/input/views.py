...
def createInfo(request):
    ...
    article.save()
    # [3] 리다이렉트
    return HttpResponsePermanentRedirect(reverse("input:index"))


def editForm(request, idx):
    # return HttPResponse ('수정페이지=' + idx)
    info = TrakingInfo.objects.get(uuid=idx)
    return render(
        request, "info_list.html", {"user1": info}
    )  # 템플릿에서 받을때 'user'로 받는다는 의미
...
def updateInfo(request):
    ...
    db_info = TrakingInfo.objects.get(uuid=idx)
...
def qrCode(request, idx):
    ...
    img = qr.make_image()
    # qr 이미지 이름 설정
    qrlink = "static/image/qr.png"
    img_resize = img.resize((256, 256))
    img_resize.save(qrlink)

    return render(request, "qr.html")


def qrkCode(request, idx):
    context = {}
    if request.POST:
        # qr 내용
        qr_data = idx
